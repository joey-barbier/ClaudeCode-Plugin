...
| qa-testing | `qa-validate agent`: Challenge implementation claims. `/qa-testing:orka-unit-test-generate`: Generate tests. |
| dev-workflow | `structured-dev-methodology agent`: For complex multi-file implementations. `/dev-workflow:orka-mvp-time-guardian`: Over-engineering detection. `/dev-workflow:orka-init-docs`: Architecture docs. `/dev-workflow:orka-git-new-feature`: Git prep. |
| cc-memory | `/cc-memory:orka-memory-restore`: Session start context restore. Update memory after significant work. |
| analytics | `saas-analytics-architect agent`: Analytics tracking strategy. |
| openclaw | `/openclaw:orka-openclaw-session-compact`: Compress sessions. `/openclaw:orka-openclaw-session-extract`: Extract learnings. |
...
- `/dev-workflow:orka-mvp-time-guardian`: When detecting over-engineering or loops
- `/cc-memory:orka-memory-restore`: Session start context restore
```
