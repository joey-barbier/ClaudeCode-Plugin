...
if [ -n "$GIT_CHANGES" ] || [ "$GIT_NEW_COMMITS" -gt 0 ]; then
  echo "IMPORTANT: Context is about to be compacted. Run /cc-memory:orka-memory-save NOW to persist session progress before context is lost. ($GIT_NEW_COMMITS recent commits, changes: $GIT_CHANGES)"
else
